...
// init() and fini() are needed for shared object
void _init() {
...
double ACOS(double pradians) {
	return acos(pradians);
}

//ASIN compute arc sine of x.
double ASIN(double pradians) {
	return asin(pradians);
}

//ATAN compute arc tangent of x.
double ATAN(double pradians) {
	return atan(pradians);
}
...
//COS
double COS(double pradians) {
	return cos(pradians);
}
//COSH  Compute the hyperbolic cosine of x.
...
// NOTE: This is a combination of the algorithms from the sites below
...
// http://www.codeforge.com/article/180627
...
double IRR(double * parray, int size) {
	...
	for (int t = 1; t < size; ++t) {
		if (sgn(parray[t - 1]) != sgn(parray[t]))
			sign_changes++;
	}

	if (sign_changes == 0)
		return ERROR;  // can not find any IRR
	...
	if (sign_changes == 1) { // IRR can be determined exactly
		...
		for (it = 1; it < iterations; it++) {
			...
			delta = log(negative / positive)
					/ (d_negative / negative - d_positive / positive);
		...
		for (i = 0; i < MAX_ITERATIONS; i++) {
			...
			if ((fabs(f_mid) < DBL_EPSILON) || (fabs(dx) < DBL_EPSILON))
				return x_mid;
		}
...
//		double oor2pi = 1 / (sqrt(double(2) * 3.14159265358979323846));
		double oor2pi = 1 / (sqrt(double(2) * my_PY));
...
//SIN
double SIN(double pradians) {
	return sin(pradians);
}

//SINH  Compute the hyperbolic sine of x.
...
//TAN
double TAN(double pradians) {
	return tan(pradians);
}

//TANH  Compute the hyperbolic tangent of x.
